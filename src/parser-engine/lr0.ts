...
export interface LR0BuildResult {
  automaton: LR0Automaton;
}

export type LR0Action =
  | {
      type: "shift";
      toState: number;
    }
...
export type LR0ActionTable = Record<
  number,
  Record<Terminal, LR0Action | undefined>
>;

export type LR0GotoTable = Record<
  number,
...
export interface LR0Conflict {
  stateId: number;
  symbol: Terminal;
  existingAction: LR0Action;
  incomingAction: LR0Action;
  type: "shift/reduce" | "reduce/reduce" | "accept/conflict";
  reason: string;
}

export interface LR0TableResult {
  action: LR0ActionTable;
  goto: LR0GotoTable;
...
export interface LR0SimulationResult {
  accepted: boolean;
  steps: LR0ParseStep[];
  error?: string;
}

export interface SimulateLR0Options {
  maxSteps?: number;
}
...
export function buildLR0Automaton(grammar: Grammar): LR0BuildResult {
  ...
  const initialItems = closure(
    grammar,
    productions,
    [{ productionId: augmentedProduction.id, dot: 0 }]
  );

  const states: LR0State[] = [];
  const stateKeyToId = new Map<string, number>();
  const queue: LR0State[] = [];

  const initialState = createState(0, initialItems, grammar, productions);
...
export function buildLR0Table(grammar: Grammar): LR0TableResult {
  const { automaton } = buildLR0Automaton(grammar);

  return buildLR0TableFromAutomaton(grammar, automaton);
}

export function buildLR0TableFromAutomaton(
  grammar: Grammar,
  automaton: LR0Automaton
): LR0TableResult {
  ...
  const action: LR0ActionTable = {};
  const goto: LR0GotoTable = {};
  const conflicts: LR0Conflict[] = [];
...
export function simulateLR0(
  grammar: Grammar,
  table: LR0TableResult,
  rawInput: string | string[],
  options: SimulateLR0Options = {}
): LR0SimulationResult {
      ...
      steps: [
        {
          ...
          action: "error",
          explanation:
            "No se puede simular porque la tabla LR(0) tiene conflictos."
        }
      ],
      error: "La tabla LR(0) tiene conflictos."
    };
  }
...
export function closure(
  ...
  const result = new Map<string, LR0Item>();
...
export function gotoLR0(
  ...
  return closure(grammar, productions, movedItems);
}
...
export function formatLR0Item(
  ...
  return `${production.left} -> ${body.join(" ")}`;
}

export function formatLR0State(
  state: LR0State,
  productions: Production[]
): string[] {
  return state.items.map((item) => formatLR0Item(item, productions));
}
...
export function formatLR0Action(action: LR0Action | undefined): string {
  if (!action) return "";
...
export function formatLR0ActionTable(result: LR0TableResult): string[][] {
  ...
  for (const state of result.automaton.states) {
    rows.push([
      ...
      ...result.terminals.map((terminal) =>
        formatLR0Action(result.action[state.id]?.[terminal])
      )
    ]);
  }
...
export function formatLR0GotoTable(result: LR0TableResult): string[][] {
...
function insertAction(
  ...
  conflicts.push({
    ...
    reason:
      `Conflicto LR(0) en ACTION[${stateId}, ${terminal}]: ` +
      `${formatLR0Action(existingAction)} contra ${formatLR0Action(incomingAction)}.`
  });
}

function sameAction(a: LR0Action, b: LR0Action): boolean {
...
function createState(
  ...
  const reductions = items
    .filter((item) => isItemComplete(item, productions))
...
function itemKey(item: LR0Item): string {
  return `${item.productionId}@${item.dot}`;
}

function itemsKey(items: LR0Item[]): string {
  return sortItems(items).map(itemKey).join("|");
}

function sortItems(items: LR0Item[]): LR0Item[] {
