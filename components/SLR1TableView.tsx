...
interface SLR1TableViewProps {
  result: SLR1TableResult;
}

export function SLR1TableView({ result }: SLR1TableViewProps) {
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-xl">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-400">
          Tabla ACTION/GOTO construida usando el autómata LR(0) y FOLLOW.
        </p>
      </div>
      ...
      <div className="mt-4 text-sm">
        {result.conflicts.length === 0 ? (
          <p className="text-emerald-300">
            La tabla no tiene conflictos. La gramática es compatible con SLR(1).
          </p>
        ) : (
...
function ActionTable({ result }: SLR1TableViewProps) {
  return (
    <div>
            ...
            {result.automaton.states.map((state) => (
              ...
              >
                ...
                {result.terminals.map((terminal) => {
                  ...
                  const formatted = formatSLR1Action(action);
...
function GotoTable({ result }: SLR1TableViewProps) {
  return (
    <div>
      <h3 className="mb-3 font-bold text-slate-100">
        GOTO
