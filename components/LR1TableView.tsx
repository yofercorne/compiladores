...
interface LR1TableViewProps {
  result: LR1TableResult;
}

export function LR1TableView({ result }: LR1TableViewProps) {
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-xl">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-400">
          Tabla ACTION/GOTO construida desde el autómata LR(1) con lookaheads.
        </p>
      </div>
      ...
      <div className="mt-4 text-sm">
        {result.conflicts.length === 0 ? (
          <p className="text-emerald-300">
            La tabla no tiene conflictos. La gramática es compatible con LR(1).
          </p>
        ) : (
...
function ActionTable({ result }: LR1TableViewProps) {
  return (
    <div>
            ...
            {result.automaton.states.map((state) => (
              ...
              >
                ...
                {result.terminals.map((terminal) => {
                  ...
                  const formatted = formatLR1Action(action);
...
function GotoTable({ result }: LR1TableViewProps) {
  return (
    <div>
      <h3 className="mb-3 font-bold text-slate-100">
        GOTO
