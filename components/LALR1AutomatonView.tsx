...
interface LALR1AutomatonViewProps {
  automaton: LALR1Automaton;
}

export function LALR1AutomatonView({
  automaton
}: LALR1AutomatonViewProps) {
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-xl">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-400">
          Estados LR(1) fusionados por núcleo LR(0), conservando sus lookaheads.
        </p>
      </div>
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    ...
                    <span className="font-mono text-slate-300">
                      {state.mergedFrom.map((id) => `I${id}`).join(", ")}
                    </span>
                  </p>
                </div>
