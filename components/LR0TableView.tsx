...
interface LR0TableViewProps {
  result: LR0TableResult;
}

export function LR0TableView({ result }: LR0TableViewProps) {
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-xl">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-400">
          Tabla ACTION/GOTO construida directamente desde el autómata LR(0).
          Las reducciones se colocan en todos los terminales.
        </p>
      </div>
      ...
      <div className="mt-4 text-sm">
        {result.conflicts.length === 0 ? (
          <p className="text-emerald-300">
            La tabla no tiene conflictos. La gramática es compatible con LR(0).
          </p>
        ) : (
...
function ActionTable({ result }: LR0TableViewProps) {
  return (
    <div>
...
{result.terminals.map((terminal) => {
  ...
  const formatted = action ? formatLR0Action(action) : "";
...
function GotoTable({ result }: LR0TableViewProps) {
  return (
    <div>
      <h3 className="mb-3 font-bold text-slate-100">
        GOTO
